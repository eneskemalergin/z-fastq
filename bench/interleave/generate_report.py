#!/usr/bin/env python3
"""Generate bench/interleave/REPORT.md and figures from zebrac JSON."""
...
FIXTURE_LABELS = {
    ...
    "casava_names": "Casava 1:N:0: / 2:N:0: pair",
    "crlf_slash": "CRLF slash /1 /2 pair",
    "empty_pair": "empty mate files",
}
...
def md_fixture_table(manifest: dict) -> str:
    ...
    if not rows_in:
        return "_No peer fixture probes were recorded in this run. Re-run `bench/interleave/run.sh` to fill this matrix._"
...
def md_capability(manifest: dict) -> str:
    peers = pd.DataFrame(
        [
            ...
            {
                "Tool": "seqtk",
                "Command": "`seqtk mergepe R1 R2`",
                "Same interleave job": "layout zip on screened LF/nonempty/bare-plus inputs; zips name mismatches",
                "Timed as": "exact layout reference",
            },
            {
                "Tool": "SeqFu",
                "Command": "`seqfu interleave -c -1 R1 -2 R2`",
                "Same interleave job": "partial; `-c` compares first tokens (`cluster1/1` ≠ `cluster1/2`; Casava first token `cluster` matches). Not z-fastq illumina.",
                "Timed as": "hatched; descriptive",
            },
            {
                "Tool": "fqkit",
                "Command": "`fqkit merge -q -@ 1 --read1 R1 --read2 R2`",
                "Same interleave job": "no; layout zip, not a name validator",
                "Timed as": "hatched; descriptive",
            },
            {
                "Tool": "IRMA Core",
                "Command": "`irma-core xleave R1 R2`",
    ...
    omitted = pd.DataFrame(
        [
            {
                "Tool": "SeqKit `pair`",
                "Why it is not here": "writes two mate files, not interleaved stdout",
            },
            {
                "Tool": "Fasten",
                "Why it is not here": "no interleave binary",
            },
            {
                "Tool": "Needletail / Helicase adapters",
                "Why it is not here": "count and stats wrappers; no interleave command",
            },
            {
                "Tool": "fastp / Rasusa / fq subsample / fqkit subfq",
                "Why it is not here": "QC or sampling, not pair interleave",
            },
            {
                "Tool": "Casava or slash timed families",
                "Why it is not here": "fixtures only; the catalog role is one mate pair",
            },
            {
                "Tool": "mixed gzip as a second race",
                "Why it is not here": "contract-only; one timed family stays pair interleave",
            },
        ]
    )
    return join(
...
def md_overview(manifest: dict) -> str:
    ...
    mates = overview_shapes(FAMILY_ROLES["pairs"])
    return join(
        [
            ...
            f"Set: **{set_name}**.",
            "",
            f"- **Pair interleave:** {mates}. Two mate files → validated interleaved stdout. Occupancy lives here.",
            "",
            "**What is timed**",
            "",
    ...
    try:
        ...
        blocks = [
            ...
            md_metric_block(
                ...
                present,
                heading="Peak memory",
                intro="Peak RSS of the timed process. These are raw process measurements; interleaver implementations do not share one memory contract.",
        ...
        if include_occupancy:
            ...
            if not occupancy.empty and scored:
                ...
                fig_occupancy_scatter(
                    ...
                    f"{kind} pair interleave occupancy: peak RSS vs wall",
                    scored,
                )
                fig_efficiency_bars(
                    occupancy,
                    figures_dir / efficiency_name,
                    f"{kind} pair interleave occupancy efficiency vs z-fastq",
                    scored,
                )
...
def generate(results_dir: Path, allow_incomplete: bool) -> None:
    ...
    if not allow_incomplete and manifest.get("verify_skipped"):
        raise SystemExit("error: publishable report requires a passing interleave contract preflight")
    ...
    if plain is not None and not plain.empty:
        lines.append(
            md_perf_section(
                plain,
                title="Performance: pair interleave",
                intro=(
                    "Catalog mate pair as interleaved stdout. Small set: DenseSmall + PairSmallR2. "
                    "Publication set: PairR1 + PairR2. Occupancy lives here. seqtk `mergepe` is the "
                    "screened exact layout reference; other interleavers are descriptive."
                ),
                tools=PLAIN_TOOLS,
                fig_name="perf_plain_pairs.png",
                fig_title="Plain pair interleave: wall, RSS, page faults",
                fig_note=f"Error bars = zebrac standard deviation (n={run_count}). Hatched = descriptive interleavers.",
                roles=FAMILY_ROLES["pairs"],
                include_occupancy=True,
                figure_caption="Wall time, peak RSS, and minor page faults for pair interleave.",
                figure_bullets=pair_bullets_plain,
                **shared_kw,
            )
        )
        lines.append("")
    if gzip is not None and not gzip.empty:
        lines.append(
            md_perf_section(
                ...
                include_occupancy=True,
                figure_caption="Wall time, peak RSS, and minor page faults for gzip pair interleave.",
                figure_bullets=pair_bullets_gzip,
                **shared_kw,
            )
        )
        lines.append("")
...
def main() -> None:
    ...
    parser.add_argument(
        "--allow-incomplete",
        default="false",
        help="true/false: allow missing perf sections or a skipped interleave contract preflight",
    )
