#!/usr/bin/env python3
"""Generate bench/check/REPORT.md and figures from zebrac JSON."""
...
FAMILY_ROLES = {
    "se": ("se",),
    "pairs": ("paired", "interleaved"),
    "names": ("paired", "casava", "slash"),
    "concat": ("gzip",),
}
FIXTURE_ORDER = [
    "iupac",
...
FIXTURE_LABELS = {
    ...
    "slash_pair": "slash /1 /2 pair",
    "casava_pair": "Casava 1:N:0: pair",
    "odd_interleaved": "odd interleaved",
    "concat_gzip": "concatenated gzip",
}
...
def configure_workloads(manifest: dict) -> None:
    ...
    for row in rows:
        ...
        expanded = ids if role == "se" and len(ids) > 1 else [None]
        for expanded_id in expanded:
            if expanded_id is None:
                key = str(row["key"])
                label_ids = ids
            else:
                key = f"se__{expanded_id}"
                label_ids = [expanded_id]
            if key in WORKLOAD_LABELS:
                continue
            if role == "se":
                label = "SE: " + ", ".join(label_ids)
            elif role == "gzip":
                label = "gzip edge: " + ", ".join(label_ids)
            else:
                label = f"{role}: " + " + ".join(label_ids)
...
def md_role_matrix(manifest: dict) -> str:
    ...
    def cell(tool: str, role: str) -> str:
        ...
        if "timed" in statuses:
            return "timed"
        if "probed, not timed" in statuses:
            return "probed, not timed"
...
def md_fixture_table(manifest: dict) -> str:
    ...
    if not rows_in:
        return "_No peer fixture probes were recorded in this run. Re-run `bench/check/run.sh` to fill this matrix._"
...
def md_capability(manifest: dict) -> str:
    peers = pd.DataFrame(
        [
            ...
            {
                "Tool": "fq",
                "Command": "`fq lint --lint-mode panic`",
                "Same check job": "no; SE and two-file pairs",
                "Timed as": "hatched; descriptive",
            },
            {
                "Tool": "FastQValidator",
                "Command": "`fastQValidator --file … --minReadLen 0 --quiet` plus `--disableSeqIDCheck` on SE or `--interleaved` on interleaved",
                "Same check job": "no; no two-file pair mode",
                "Timed as": "hatched; descriptive",
            },
            {
                "Tool": "SeqFu",
                "Command": "`seqfu check --deep --quiet` (`--no-paired` on one file)",
                "Same check job": "no; SE and two-file pairs",
                "Timed as": "hatched; RSS not occupancy",
            },
            {
                "Tool": "fqtools",
                "Command": "`fqtools -d -a -u -l -q s validate` (`-i` when interleaved)",
                "Same check job": "no; extra DNA/empty/uniqueness/Sanger policy",
                "Timed as": "hatched; descriptive",
            },
        ]
    )
    omitted = pd.DataFrame(
        [
            {
                "Tool": "seqtk",
                "Why it is not here": "no validator; count suite only",
            },
            {
                "Tool": "SeqKit `seq --validate-seq`",
                "Why it is not here": "alphabet check, not FASTQ structure or pair names",
            },
            {
                "Tool": "Needletail / Helicase adapters",
                "Why it is not here": "count and stats wrappers; no check command",
            },
            {
                "Tool": "fastp",
                "Why it is not here": "QC and transformation, not a gate validator",
            },
            {
                "Tool": "BBTools",
                "Why it is not here": "JVM pair utilities; RSS is not a comparable process image",
            },
        ]
    )
    return join(
...
def md_overview(manifest: dict) -> str:
    ...
    se_names = ", ".join(display_workload(key) for key in family_workloads(("se",))) or "none"
    pair_names = ", ".join(display_workload(key) for key in family_workloads(("paired", "interleaved"))) or "none"
    name_names = ", ".join(display_workload(key) for key in family_workloads(("paired", "casava", "slash"))) or "none"
    concat_names = ", ".join(display_workload(key) for key in family_workloads(("gzip",))) or "none"
    return join(
        [
    ...
    try:
        ...
        blocks = [
            ...
            md_metric_block(
                ...
                present,
                heading="Peak memory",
                intro="Peak RSS of the timed process. These are raw process measurements; validator implementations do not share one memory contract.",
        ...
        if include_occupancy:
            ...
            if not occupancy.empty and scored:
                ...
                fig_occupancy_scatter(
                    ...
                    f"{kind} SE check occupancy: peak RSS vs wall",
                    scored,
                )
                fig_efficiency_bars(
                    occupancy,
                    figures_dir / efficiency_name,
                    f"{kind} SE check occupancy efficiency vs z-fastq",
                    scored,
                )
...
def generate(results_dir: Path, allow_incomplete: bool) -> None:
    ...
    if not allow_incomplete and manifest.get("verify_skipped"):
        raise SystemExit("error: publishable report requires a passing check contract preflight")
    ...
    if plain is not None and not plain.empty:
        lines.append(
            md_perf_section(
                ...
                figure_caption="Wall time, peak RSS, and minor page faults for SE check workloads.",
                figure_bullets=se_bullets,
                **shared_kw,
            )
        )
        lines.append("")
    if gzip is not None and not gzip.empty:
        lines.append(
            md_perf_section(
                ...
                include_occupancy=True,
                figure_caption="Wall time, peak RSS, and minor page faults for gzip SE check workloads.",
                figure_bullets=se_bullets,
                **shared_kw,
            )
        )
    ...
    if plain is not None and not plain.empty:
        lines.append(
            md_perf_section(
                plain,
                title="Performance: pair layout",
                intro="`--paired` vs `--interleaved` on the matching mates (same pair count). SE DenseSmall is half the records of this pair and is not plotted here as pair overhead. Tables and a 3-facet bar only; no occupancy.",
                tools=PLAIN_TOOLS,
                fig_name="perf_plain_pairs.png",
                fig_title="Plain pair layout check: wall, RSS, page faults",
                fig_note=f"Error bars = zebrac standard deviation (n={sample_count}). Hatched = external validators.",
                roles=FAMILY_ROLES["pairs"],
                figure_caption="Wall time, peak RSS, and minor page faults for paired vs interleaved check.",
                figure_bullets=pair_bullets,
                **shared_kw,
            )
        )
        lines.append("")
    ...
    if plain is not None and not plain.empty:
        lines.append(
            md_perf_section(
                plain,
                title="Performance: name policy",
                intro="Paired (SRA-style names) vs Casava `1:N:0:` / `2:N:0:` vs `@id/1` / `@id/2`. Same MiniSeq sequences; only headers change. This is a header-policy story, not a volume race. Tables and a compact 3-facet; no occupancy.",
                tools=PLAIN_TOOLS,
                fig_name="perf_plain_names.png",
                fig_title="Plain name-policy check: wall, RSS, page faults",
                fig_note=f"Error bars = zebrac standard deviation (n={sample_count}). Hatched = external validators.",
                roles=FAMILY_ROLES["names"],
                figure_caption="Wall time, peak RSS, and minor page faults for pair-name policy check.",
                figure_bullets=name_bullets,
                **shared_kw,
            )
        )
        lines.append("")
...
def main() -> None:
    ...
    parser.add_argument(
        "--allow-incomplete",
        default="false",
        help="true/false: allow missing perf sections or a skipped check contract preflight",
    )
