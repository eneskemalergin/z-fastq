#!/usr/bin/env python3
"""Generate bench/stats/REPORT.md and figures from zebrac JSON."""
...
DATASET_ORDER = ["DenseSmall", "Variable", "Long", "HiFi", "PairR1"]
DATASET_FACTS = {
    "SRR10325788": (
        "SRR10325788 MiniSeq RNA-seq R1, 60,910 x 153 bp, real Illumina quality. "
        "The R2 mate is PairSmallR2 and is not timed here."
    ),
    "ERR164407": (
    ...
    "ERR5404925": (
        "ERR5404925 MiSeq R1, 349,760 reads, trimmed 30-151 bp. PairR2 is the mate and is not timed here."
    ),
    "SRR1810900": (
        "SRR1810900, HiSeq 2500 ChIP-seq, 24.5 million 50 bp reads. Every quality byte is `?`. "
        "Omitted from stats."
    ),
}
BASELINE = "z-fastq"
REFERENCE_TOOLS = frozenset({"seqfu", "seqkit"})
NO_RSS_RATIO = frozenset({"seqfu", "seqkit"})

# Complete stats job first, then field-stats CLIs that are not the same job.
PLAIN_TOOLS = [
    "z-fastq",
    "needletail",
    "helicase",
...
# Isocost dash follows the scatter shape.
DATASET_LINESTYLES = {
    "DenseSmall": "-",
    "Variable": "--",
    "Long": "-.",
    "HiFi": ":",
    "PairR1": (0, (8, 3, 2, 3, 2, 3)),
}
DATASET_LINE_NAMES = {
    "DenseSmall": "solid",
    "Variable": "dashed",
...
def dataset_shape_caption(datasets: list[str]) -> str:
    parts = []
    for name in datasets:
        marker = DATASET_MARKER_NAMES.get(name, "mark")
        line = DATASET_LINE_NAMES.get(name, "line")
        parts.append(f"{name} {marker}/{line}")
    return ", ".join(parts)
...
def fig_occupancy_scatter(frame: pd.DataFrame, out: Path, title: str, *, alpha: float = 0.5) -> Path:
    ...
    fig.text(
        ...
        (
            ...
            f"({dataset_shape_caption(datasets)}). Gold 1x, gray 2x/4x/8x. Lower-left is better."
        ),
        ha="center",
        va="top",
        fontsize=8,
    ...
    fig.subplots_adjust(left=0.11, right=0.985, bottom=0.30 if len(datasets) > 3 else 0.27, top=0.885)
...
def fig_efficiency_bars(frame: pd.DataFrame, out: Path, title: str) -> Path:
    ...
    if not tools or not datasets:
        ...
        ax.set_axis_off()
        return _save(fig, out)
    hatches = [DATASET_HATCHES.get(dataset, "") for dataset in datasets]
    width = min(0.18, 0.72 / max(1, len(datasets)))
    x = list(range(len(tools)))
    fig, ax = plt.subplots(figsize=(12.2 if len(datasets) > 3 else 10.5, 5.6))
...
def timed_datasets(manifest: dict) -> dict:
...
def md_overview(manifest: dict) -> str:
    slots = timed_datasets(manifest)
    present = [name for name in DATASET_ORDER if name in slots]
    names = ", ".join(present) if present else "the timed catalog ids"
    verify = manifest.get("verify_pass")
    if manifest.get("verify_skipped"):
        verify_line = (
            "The stats-agreement check was skipped (`--skip-tests`). Do not treat this as a published run."
        )
    elif verify:
        verify_line = (
        ...
        verify_line = "The stats-agreement check was not recorded."
    omitted = (
        "Dense (SRR1810900) is omitted: every quality byte is `?`, so Q20/Q30 and mean quality are not a "
        "quality-score job."
    )
    return join(
        [
            "This report times `z-fastq stats` on matched plain and gzip FASTQ. Stats uses catalog ids, "
            f"not the count Dense/Variable/Long slots. Timed here: {names}. {omitted} "
...
def md_capability() -> str:
    peers = pd.DataFrame(
        [
            ...
            {
                "Tool": "Needletail",
                "Command": "`needletail-adapter stats`",
                "Same stats job": "yes, same human fields",
                "Timed as": "equal-work wrapper",
            },
            {
                "Tool": "Helicase",
                "Command": "`helicase-adapter stats`",
                "Same stats job": "yes, same human fields",
                "Timed as": "equal-work wrapper",
            },
            {
                "Tool": "SeqKit",
                "Command": "`seqkit stats -a -T -j 1`",
                "Same stats job": "no; lengths, GC%, Q20%, Q30%; AvgQual is error-mean Q",
                "Timed as": "hatched; RSS not occupancy",
            },
            {
                "Tool": "SeqFu",
                "Command": "`seqfu stats --threads 1 --gc`",
                "Same stats job": "no; lengths and GC; default is 8 threads",
    ...
    omitted = pd.DataFrame(
        [
            {
                "Tool": "seqtk `size`",
                "Why it is not here": "reads and bases only; that is the count suite",
            },
            {
                "Tool": "seqtk `fqchk`",
                "Why it is not here": "drops empty sequences; positional quality histogram, not per-base Q20/Q30",
            },
            {
                "Tool": "fqtools",
                "Why it is not here": "count only",
            },
            {
                "Tool": "fqkit `stats`",
                "Why it is not here": "writes files, logs to stderr, panics on an empty sequence",
            },
            {
                "Tool": "fqkit `size`",
                "Why it is not here": "reads/bases/ACGTN plus stderr logs; not quality",
            },
        ]
    )
    fields = pd.DataFrame(
        [
            {
                "Field": "reads / bases / min / max / mean length",
                "z-fastq + adapters": "yes",
                "SeqKit -a": "yes (avg_len rounded)",
                "SeqFu --gc": "yes (Avg rounded)",
            },
            {
                "Field": "A/C/G/T/N / other_bases",
                "z-fastq + adapters": "yes",
                "SeqKit -a": "sum_n only",
                "SeqFu --gc": "no",
            },
            {
                "Field": "GC",
                "z-fastq + adapters": "fraction over A+C+G+T",
                "SeqKit -a": "GC%",
                "SeqFu --gc": "%GC, 2 decimals",
            },
            {
                "Field": "quality_sum / mean_quality",
                "z-fastq + adapters": "arithmetic Phred+33 mean",
                "SeqKit -a": "AvgQual is -10 log10(mean error p)",
                "SeqFu --gc": "no",
            },
            {
                "Field": "q20 / q30",
                "z-fastq + adapters": "bases and fraction",
                "SeqKit -a": "integer percent",
                "SeqFu --gc": "no",
            },
        ]
    )
    return join(
        [
            "`z-fastq stats` prints one human block: reads, bases, length min/max/mean, ACGTN, other_bases, "
            "GC over A+C+G+T, quality_sum, arithmetic mean Phred, q20/q30 bases and fractions. That is the compared fact.",
            "",
            to_markdown_aligned(peers, index=False),
            "",
            "SeqKit AvgQual on the fixture is ~6 while arithmetic mean Phred is ~31. That is a different formula, "
            "not a bug in either tool. Occupancy uses only complete peers.",
            "",
            "**Not timed**",
            "",
            ...
            "**Fields**",
            "",
            to_markdown_aligned(fields, index=False),
        ]
    )
...
def md_correctness(manifest: dict) -> str:
    ...
    return join(
        [
            "Valid four-line FASTQ only. SeqKit and SeqFu are not error-status references. Malformed fixtures "
            "are a separate z-fastq reject check.",
            "",
            body,
            "",
...
def md_provenance(manifest: dict) -> str:
    ...
    if manifest.get("verify_skipped"):
        check_line = "- **Stats check:** skipped (`--skip-tests`)"
    elif verify and log:
        check_line = f"- **Stats check:** {verify} (`{log}`)"
    elif verify:
        check_line = f"- **Stats check:** {verify}"
    else:
        check_line = "- **Stats check:** not recorded"
    parts = [
...
def md_perf_section(
    ...
    *,
    ...
    blocks = [
        ...
        md_metric_block(
            heading="Wall time",
            intro="Zebrac mean wall time for one `stats` / peer process with stdout discarded.",
            work=df,
            tools=present,
            value_col="mean",
            fmt=fmt_wall,
    ...
    blocks.extend(
        [
            md_metric_block(
                heading="Memory",
                intro="Peak RSS of the timed process. Zebrac starts the tool directly and does not start a shell. SeqKit and SeqFu are omitted from RSS ratios: they are not the same stats job, SeqFu may launch helpers, and SeqKit RSS is the Go image.",
                work=df,
                tools=present,
    ...
    if not occ_frame.empty:
        ...
        occ_ids = [d for d in DATASET_ORDER if d in set(occ_frame["dataset"])]
        blocks.extend(
            [
...
def md_run_banner(manifest: dict) -> str | None:
    ...
    if manifest.get("real_set") == "small":
        notes.append("small set: DenseSmall, Variable, HiFi (publication also times Long and PairR1)")
    try:
...
def generate(results_dir: Path, allow_incomplete: bool) -> None:
    ...
    if not allow_incomplete and manifest.get("verify_skipped"):
        raise SystemExit("error: publishable report requires a passing stats-agreement check")
    figures_dir = results_dir / "figures"
    ...
    if plain is not None and not plain.empty:
        lines.append(
            md_perf_section(
                ...
                fig_title="Plain FASTQ stats: wall, RSS, page faults",
...
def main() -> None:
    ...
    ap.add_argument(
        "--allow-incomplete",
        default="false",
        help="true/false: allow missing perf sections or a skipped stats-agreement check",
    )
    args = ap.parse_args()
