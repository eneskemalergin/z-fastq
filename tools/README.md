...
Every retained native executable is stripped, and `--check` verifies this. BBTools contains Java class files and shell scripts, so native executable stripping does not apply to it. SeqFu builds from its tagged source with a temporary pinned Nim compiler and locked packages. Nim, Nimble, and their build files are discarded afterward.
...
| fq                               |               0.12.0 | 2.6 MiB | Main shared-rule validation and sampling peer                 |
| fastp                            |                1.3.6 | 2.9 MiB | Broader short-read QC and preprocessing context               |
| Rasusa                           |                5.1.0 | 5.4 MiB | Probability, exact-count, base, and coverage sampling         |
...
| BBTools                          |                40.01 |  22 MiB | Reformat, pair-aware sampling, and read-pair repair           |

The complete generated tool area is about 62 MiB, plus a 96 KiB tools-only Python environment.
